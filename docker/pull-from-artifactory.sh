...
echo "fetch "$repo_url/$(echo "$group" | tr . /)/$artifact/$version/$artifact-$version.war""
curl -k -o $output -X GET "$repo_url/$(echo "$group" | tr . /)/$artifact/$version/$artifact-$version.war"
